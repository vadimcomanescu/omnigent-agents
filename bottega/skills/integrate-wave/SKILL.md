...
rewritten code back to the owning slice's session.
...
that slice's OWN session (per run-slice-pipeline's feedback rule) to fold the
helper into one home, with the other slice consuming it; let it re-hand-back, then
merge. Never edit, consolidate, or refactor helpers inline.
...
- Re-run the gates (tests / lint / typecheck, coverage if configured) and confirm
  ...
  anything else is BOUNCED to the owning slice's session. Never hand-edit source
  to force a gate green yourself.
...
- **Anything beyond mechanical** — overlapping logic, incompatible shapes, a
  ...
  NOT yours. Bounce it to the OWNING slice's OWN session (continue it, per
  run-slice-pipeline's feedback rule) to rebase onto the new integration HEAD and
