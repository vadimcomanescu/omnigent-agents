...
---
<!-- Canonical source: maestro/_shared-skills/qa-verify/SKILL.md. The per-agent copies at maestro/agents/{claude_code,codex,pi}/skills/qa-verify/SKILL.md are real-file duplicates (not symlinks, so the bundle uploader packages and the extractor accepts them). Edit the canonical file, then regenerate the three copies. -->
