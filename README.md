...
| [`maestro`](./maestro) | **maestro — multi-agent coding orchestrator (tech lead).** A fork of Omnigent's bundled `polly` example, renamed for independent divergence. Breaks a goal into pieces and delegates **every** code/test change, investigation, and review to three coding sub-agents (`claude_code`, `codex`, `pi`) — each in its own git worktree opening its own PR — then has a **different-vendor** sub-agent cross-review each diff (diff + contract only). Writes no code itself and **never merges**; the human merges. Best for bigger tasks you want planned, split up, and independently verified. |
...
├── maestro/       # config.yaml + agents/<name>/ + skills/<name>/ + README (fork of omnigent's polly)
└── README.md
```
