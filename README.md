...
| Team | What it's for |
|------|---------------|
| [`duetta`](./duetta) | **Duetta, two-voice brainstorming.** Sends every question to both a Claude responder and a GPT responder, then shows the two perspectives side by side. The GPT voice uses the local Codex CLI harness instead of the OpenAI Agents SDK, so it runs through `codex login` rather than `OPENAI_API_KEY`. The optional `debate` skill has the two voices critique each other before a synthesis. |
