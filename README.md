...
| [`maestro`](./maestro) | **Maestro — a delegating tech-lead.** Decomposes a goal and hands every coding task to a **different-vendor** sub-agent (Claude Code / Codex / Pi), each in its own git worktree, then verifies every PR with an **independent, opposite-vendor** reviewer that sees only the diff + acceptance contract. Maestro writes no code itself and **never merges** — each implementer opens its own PR for a human to merge. A leaner, human-gated counterpart to `bottega`: best for bigger tasks you want planned, split up, and cross-checked rather than run fully unattended. |
...
├── maestro/        # config.yaml + agents/<name>/ + skills/<name>/
└── README.md
```
